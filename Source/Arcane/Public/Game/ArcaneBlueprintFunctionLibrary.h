...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Arcane|SaveGame")
	static TArray<FString> FindAllSaveGames(FString SlotPrefix = FString("UMS"));

	// 传入角色名（FName)，传入插槽名前缀(默认FString("UMS")），创建一个新的游戏存档
	UFUNCTION(BlueprintCallable, Category = "Arcane|SaveGame")
	static void CreateNewGameSave(FName PlayerName, FString SlotPrefix = FString("UMS"));
