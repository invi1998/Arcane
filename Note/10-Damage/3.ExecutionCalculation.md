...
# ExecCalcs可以修改多个属性，但MMCs不能

ExecCalcs可以修改多个属性，但MMCs不能。在虚幻引擎（Unreal Engine）中，`ExecCalcs` 可以修改多个属性，而 `MMCs`（即 `Multi-Member Capture`）则只能捕捉单个属性。

`ExecCalcs` 是游戏效果（Gameplay Effect）执行计算的一部分，它允许开发者在游戏效果应用到游戏对象时执行自定义的代码逻辑。通过 `ExecCalcs`，开发者可以访问和修改游戏对象的属性，从而实现复杂的计算和行为。

相比之下，`MMCs` 是游戏效果（Gameplay Effect）属性捕捉的一部分，它允许开发者从游戏对象中捕捉属性值并应用到游戏效果中。然而，每个 `MMCs` 只能捕捉一个属性，因此无法同时处理多个属性的捕捉。

因此，虽然 `ExecCalcs` 和 `MMCs` 都可以用于游戏效果的属性处理，但它们在处理属性的数量上有显著的区别。`ExecCalcs` 可以处理多个属性，而 `MMCs` 只能处理单个属性。